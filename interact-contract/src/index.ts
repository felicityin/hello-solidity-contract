...
async function main() {
    ...
    const contract = new ethers.Contract(contractAddress, lockContract.abi, signer);

    console.log('before calling the contract: ', await provider.getBalance(contract.target));

    // Call the contract functions
    await contract.withdraw();

    console.log('after calling the contract: ', await provider.getBalance(contract.target));
